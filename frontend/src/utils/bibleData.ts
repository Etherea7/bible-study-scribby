 ...
 * Bible Data
 *
 * Complete verse counts for all 66 books of the Bible.
 * Used for client-side validation of verse ranges.
 */

// Verse counts per chapter for each book
export const BIBLE_VERSES: Record<string, number[]> = {
...
export const BIBLE_BOOKS = Object.entries(BIBLE_VERSES).map(([name, chapters]) => ({
  name,
...
export function getVerseCount(book: string, chapter: number): number {
  const bookVerses = BIBLE_VERSES[book];
...
export function getChapterCount(book: string): number {
  const bookVerses = BIBLE_VERSES[book];
  return bookVerses ? bookVerses.length : 0;
}
